              ...
              `}
				...
				<div className="sm:hidden flex flex-1 justify-end items-center">
					...
					<div
						className={`${
							!toggle ? "hidden" : "flex"
						} bg-[#222] absolute top-20 mx-4 my-2 min-w-[140px] z-10 rounded-xl`}
					>
						<ul className="w-full flex flex-col justify-end items-start ">
							{navLinks.map((link) => (
								<li
									...
									className={`
                  ${active == link.title ? "text-white" : "text-secondary"}
                    hover:text-white text-md font-medium cursor-pointer px-4 py-3 hover:bg-white/10 w-full border-b border-[#666666] last:border-0 first:rounded-t-xl last:rounded-b-xl
                  `}
									onClick={() => {
